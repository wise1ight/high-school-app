...
class AJXP_SqlUser extends AbstractAjxpUser
{
    ...
    public function load()
    {
        ...
        // update group
        $res = dibi::query('SELECT [groupPath] FROM [xe_member] WHERE [user_id] = %s', $this->getId());
    ...
    public function setGroupPath($groupPath, $update = false)
    {
        if ($update &&  isSet($this->groupPath) && $groupPath != $this->groupPath) {
            ...
            $res = dibi::query("SELECT [u.user_id] FROM [xe_member] AS u, [ajxp_user_rights] AS p WHERE [u.login] = [p.login] AND [p.repo_uuid] = %s AND [p.rights] = %s AND [u.groupPath] != %s ", "ajxp.parent_user", $this->getId(), $groupPath);
            foreach ($res as $row) {
                $userId = $row->login;
        ...
        dibi::query('UPDATE [xe_member] SET ', Array('groupPath'=>$groupPath), 'WHERE [user_id] = %s', $this->getId());
