...
class sqlConfDriver extends AbstractConfDriver
{
    ...
    {
        $ignoreHiddens = "NOT EXISTS (SELECT * FROM [ajxp_user_rights] WHERE [ajxp_user_rights.login]=[xe_member.user_id] AND [ajxp_user_rights.repo_uuid] = 'ajxp.hidden')";
        $children = array();
        $children_results = dibi::query('SELECT [xe_member].[user_id] FROM [ajxp_user_rights],[xe_member] WHERE [repo_uuid] = %s AND [rights] = %s AND [ajxp_user_rights].[login] = [xe_member].[user_id] AND '.$ignoreHiddens, "ajxp.parent_user", $userId);
        $all = $children_results->fetchAll();
        foreach ($all as $item) {
            $children[] = $this->createUserObject($item["user_id"]);
        }
        return $children;
    ...
    public function getUsersForRepository($repositoryId)
    {
        $result = array();
        // OLD METHOD
        $children_results = dibi::query('SELECT [xe_member].[user_id] FROM [ajxp_user_rights],[xe_member] WHERE [repo_uuid] = %s AND [ajxp_user_rights].[login] = [xe_member].[user_id] GROUP BY [xe_member].[user_id]', $repositoryId);
        $all = $children_results->fetchAll();
        foreach ($all as $item) {
            $result[$item["user_id"]] = $this->createUserObject($item["user_id"]);
        }
    ...
    public function deleteGroup($groupPath)
    {
        ...
        $res = dibi::query("SELECT * FROM [xe_member] WHERE [groupPath] LIKE %like~ OR [groupPath] = %s ORDER BY [user_id] ASC", $groupPath."/", $groupPath);
        $rows = $res->fetchAll();
        $subUsers = array();
        foreach ($rows as $row) {
            $this->deleteUser($row["user_id"], $subUsers);
            dibi::query("DELETE FROM [xe_member] WHERE [user_id] = %s", $row["user_id"]);
        }
    ...
    public function deleteUser($userId, &$deletedSubUsers)
    {
        ...
        try {
            ...
            //dibi::query('DELETE FROM [xe_member] WHERE [login] = %s', $userId);
    ...
    public function listUserTeams()
    {
        ...
        dibi::query("DELETE FROM [ajxp_user_teams] WHERE [user_id] NOT IN (SELECT [user_id] FROM [xe_member])");
