<?php
     ...
     * @brief  동아리 모듈의 admin controller class
     **/

     class clubAdminController extends club {
         ...
         * @brief 카페 설정
         **/
        function procClubAdminInsertConfig() {
            global $lang;
            ...
            if($site_srl) {
                $oModuleController->insertModulePartConfig('club', $site_srl, $args);
                $club_info = $oClubModel->getClubInfo($site_srl);
				
				unset($args);
				$args->site_srl = $vars->site_srl;
				$args->default_layout = $vars->default_layout;
				$this->insertLayout($args,$club_info);
				$args->default_layout = $vars->default_mlayout;
				$this->insertLayout($args,$club_info,'M');
            ...
            // 기본 정보 인 경우
                ...
                $club_config = $oClubModel->getConfig(0);
                $mid = $club_config->cafe_main_mid;
				...
				$oModuleController->insertModuleConfig('club', $args);
            }
        }

		function insertLayout($args,$club_info,$layout_type = "P")
		{
			$oLayoutModel = &getModel('layout');
			$oLayoutAdminController = &getAdminController('layout');
			if($layout_type != 'M')
				$layout_srl = $club_info->layout_srl;
			else
				$layout_srl = $club_info->mlayout_srl;
			...
			if(!$layout_info || $layout_info->layout != $args->default_layout) 
			{
				...
				else
				{
					...
					$extra_vars->main_menu = $club_info->first_menu_srl;
					$extra_vars->logo_text = $club_info->title;
					...
					$layout_args->title = $club_info->title;
				...
				$output = executeQuery('club.updateClub', $home_args);
				return $output;
			}
			return;
		}
         ...
         **/
        function procClubAdminInsertClub() {
            ...
            $output = $this->insertClub($title, $domain);
			...
			if (Context::get('success_return_url') || !$this->get('site_srl')){
				...
				$this->setRedirectUrl(getNotEncodedUrl('', 'module', 'admin', 'act', 'dispClubAdminSetup', 'site_srl', $this->get('site_srl')));
			}
        ...
        function insertClub($title, $domain) {
            $oModuleController = &getController('module');
			...
			foreach($supported_lang as $lang_prefix => $lang_text)
			{
				...
				Context::loadLang(_XE_PATH_.'modules/club/lang/');
            ...
            $club_config = $oClubModel->getConfig(0);
            if(!$club_config->default_layout) $club_config->default_layout = 'xe_cafe_site';

            // 레이아웃 생성
            $info->layout_srl = $this->makeLayout($info->site_srl, $title,$club_config->default_layout);
            $info->mlayout_srl = $this->makeLayout($info->site_srl, $title,$club_config->default_mlayout,'M');
            ...
            $output = executeQuery('club.insertClub', $args);
			...
			$oModuleController = &getController('module');
			unset($args);
			$args->default_layout = $club_config->default_layout;
			$args->default_mlayout = $club_config->default_mlayout;
			$args->enable_change_layout = $club_config->enable_change_layout;

			$args->use_mobile = $club_config->use_mobile;
			$args->mskin = $club_config->mskin;
			$args->mlayout_srl = $club_config->mlayout_srl;
			$args->allow_service = $club_config->allow_service;
        ...
        function procClubAdminUpdateClub() {
            $oClubModel = &getModel('club');
            $oModuleController = &getController('module');

            // 카페이름, 접속방법, 카페관리자 지정
            $args = Context::gets('site_srl','title','club_admin','layout_srl');
            ...
            $club_info = $oClubModel->getClubInfo($args->site_srl);
			if(!$club_info->site_srl) return new Object(-1,'msg_invalid_request');

            // 관리자 지정
            $admin_list = explode(',',$args->club_admin);
            ...
            $this->procClubAdminInsertConfig();

			// 도메인 변경된 경우 캐시파일 재생성
			if($club_info->domain != $args->domain && $club_info->first_menu_srl) {
				$oMenuAdminController = &getAdminController('menu');
				$oMenuAdminController->makeXmlFile($club_info->first_menu_srl);
			}

			$this->setMessage('success_updated');
        }

        function procClubAdminDeleteClub() {
            ...
            // 홈페이지 정보 삭제
            executeQuery('club.deleteClub', $args);
            ...
            // 메뉴 삭제
            $oMenuAdminController = &getAdminController('menu');
            $oMenuAdminController->deleteMenu($club_info->first_menu_srl);
            ...
            // 레이아웃 삭제
            Context::set('layout_srl', $club_info->layout_srl);
            $oLayoutAdminController = &getAdminController('layout');
            $oLayoutAdminController->procLayoutAdminDelete();

			//모바일 레이아웃 삭제
			Context::set('layout_srl', $club_info->mlayout_srl);
            $oLayoutAdminController = &getAdminController('layout');
            $oLayoutAdminController->procLayoutAdminDelete();
         ...
         **/
        function procClubAdminImportModule() {
            $oModuleModel = &getModel('module');
            $oModuleController = &getController('module');
            $oClubModel = &getModel('club');
            $oMenuAdminController = &getAdminController('menu');
            ...
            // 대상 모듈의 site_srl을 변경
            $output = $oModuleController->updateModuleSite($module_srl, $site_srl, $club_info->layout_srl);
            if(!$output->toBool()) return $output;


            // 대상 모듈을 최하단 메뉴로 추가
            $args->menu_srl = $club_info->first_menu_srl;
            $args->menu_item_srl = getNextSequence();
            $args->parent_srl = 0;
         ...
         **/
        function procClubAdminExportModule() {
            $oModuleModel = &getModel('module');
            $oModuleController = &getController('module');
            $oClubModel = &getModel('club');
            $oMenuAdminController = &getAdminController('menu');
			...
			// 내보낸 모듈관 연결된 메뉴 삭제
			$args->target_menu_srl = $club_info->first_menu_srl;
			$args->target_mid = $module_info->mid;
			$output = executeQueryArray('club.getMenuItemByMenuSrlAndMid', $args);
