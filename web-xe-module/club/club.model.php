    ...
    class clubModel extends club {
        ...
        function getConfig($site_srl = 0) {
            ...
            if(!isset($configs[$site_srl])) {
                ...
                if($site_srl) {
                    $part_config = $oModuleModel->getModulePartConfig('club', $site_srl);
        ...
        function getClubInfo($site_srl) {
            static $infos = array();
            if(!isset($infos[$site_srl])) {
                $args->site_srl = $site_srl;
                $output = executeQuery('club.getClubInfo', $args);
		...
		function getClubMenuTplInfo(){
            ...
            $club_config = $this->getConfig($this->site_srl);
            if(count($club_config->allow_service)) {
                foreach($club_config->allow_service as $k => $v) {
                    if($v<1) continue;
                    $c = $oModuleModel->getModuleCount($this->site_srl, $k);
                    $club_config->allow_service[$k] -= $c;
                }
            }
            ...
            $tpl = $oTemplate->compile($this->module_path.'tpl', 'club_menu_item_info');
