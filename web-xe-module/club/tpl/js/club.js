...
function doUpdateClub(fo_obj, func) {
    var sel_obj = fo_obj.admin_list;
    var arr = new Array();
    for(var i=0;i<sel_obj.options.length;i++) {
        arr[arr.length] = sel_obj.options[i].value;
    }
    fo_obj.club_admin.value = arr.join(',');
    procFilter(fo_obj, func);
    return false;

}

function completeUpdateClub(ret_obj) {
    alert(ret_obj['message']);
    location.reload();
}

function completeDeleteClub(ret_obj) {
    alert(ret_obj['message']);
    location.href = current_url.setQuery('act','dispClubAdminContent').setQuery('site_srl','');
}
...
function completeInsertBoard(ret_obj) {
    ...
    var url = current_url.setQuery('act','dispClubBoardInfo');
...
function doRemoveMember(confirm_msg) {
    ...
    exec_xml('club','procClubDeleteMember', params, doCompleteRemoveMember);
}
...
function insertSelectedModule(id, module_srl, mid, browser_title) {
    ...
    exec_xml('club','procClubAdminImportModule', params, doComplenteInsertSelectedModule);
}
