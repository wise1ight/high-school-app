<?php
     ...
     * @brief  동아리 모듈의 admin view class
     **/

    class clubAdminView extends club {
        ...
        function init() {
            if(strpos($this->act,'ClubAdminSite')!==false) {
                ...
                Context::set('club_info', $this->club_info);
        ...
        function dispClubAdminInsert() {
            Context::addJsFilter($this->module_path.'tpl/filter', 'insert_club.xml');
            $this->setTemplateFile('insert');

        }

        function dispClubAdminManage() {
            ...
            Context::set('club_config', $club_config);
        ...
        function dispClubAdminSetup() {
            ...
            Context::set('club_info', $club_info);

            // cafe 전체 설정을 구함
            $club_config = $oClubModel->getConfig($site_srl);
            Context::set('club_config', $club_config);
        ...
        function dispClubAdminSkinSetup() {
            $oModuleAdminModel = &getAdminModel('module');
            $oClubModel = &getModel('club');

            $club_config = $oClubModel->getConfig(0);
            $skin_content = $oModuleAdminModel->getModuleSkinHTML($club_config->module_srl);
            Context::set('skin_content', $skin_content);

            $this->setTemplateFile('skin_info');
        }
        function dispClubAdminMobileSkinSetup() {
            $oModuleAdminModel = &getAdminModel('module');
            $oClubModel = &getModel('club');

            $club_config = $oClubModel->getConfig(0);
            $skin_content = $oModuleAdminModel->getModuleMobileSkinHTML($club_config->module_srl);
            Context::set('skin_content', $skin_content);
         ...
         * @brief 홈페이지 기본 관리
         **/
        function dispClubAdminSiteManage() {
            $oModuleModel = &getModel('module');
            $oMenuAdminModel = &getAdminModel('menu');
            $oLayoutModel = &getModel('layout');
            $oClubModel = &getModel('club');
            ...
            if(!Context::get('act')) Context::set('act', 'dispClubManage');
         ...
         * @brief 홈페이지 회원 그룹 관리
         **/
        function dispClubAdminSiteMemberGroupManage() {
            // 멤버모델 객체 생성
            $oMemberModel = &getModel('member');
         ...
         **/
        function dispClubAdminSiteMemberManage() {
            $oMemberModel = &getModel('member');
			$oModuleModel = &getModel('module');
         ...
         * @brief 홈페이지 상단 메뉴 관리
         **/
        function dispClubAdminSiteTopMenu() {
            $oMemberModel = &getModel('member');
            ...
            // 메뉴 정보 가져오기
            $menu_srl = $this->club_info->first_menu_srl;

            $menu_info = $oMenuModel->getMenu($menu_srl);
            Context::set('menu_info', $menu_info);


            $selected_layout = $oLayoutModel->getLayout($this->club_info->layout_srl);
         ...
         * @brief 애드온/ 컴포넌트 설정
         **/
        function dispClubAdminSiteComponent() {
            // 애드온 목록을 가져옴
         ...
         * @brief 접속 통계
         **/
        function dispClubAdminSiteCounter() {
            // 정해진 일자가 없으면 오늘자로 설정
         ...
         * @brief 홈페이지 모듈 목록
         **/
        function dispClubAdminSiteMidSetup() {
